        ...
        <div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content">
                <main>
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $answer->user->name }}</td> <!-- Asumsi kolom name ada di tabel users -->
                        <td>{{ $answer->user->id }}</td>
                        <td>{{ $answer->description }}</td> <!-- Asumsi kolom judul ada di tabel tugas -->
                        <td>
                            @if ($answer->imageAnswer)
                                <img src="{{ asset($answer->imageAnswer) }}" alt="Gambar Jawaban" width="100">
                            @else
                                Tidak ada gambar
                            @endif
                        </td>
                        <td>
                            <form action="{{ route('admin.jawaban.hapus', $answer->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
